<?php 
    include_once('connect.php');

    $result = mysqli_query($conn, 'SELECT * FROM Buku_table ORDER BY id_buku DESC');
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <title>Document</title>
    <style>
    footer {
    text-align: center;
    padding: 3px;
    color: black;
    }
</style>
</head>

<body>
    <nav class="navbar navbar-dark bg-dark justify-content-between">
        <a class="navbar-brand" href="home.php">
        <img src="./images/logo-ead-1.png" width="200" />
        </a>
        <a class="btn btn-primary my-2 text-white my-sm-0" href="create.php" type="submit">Tambah Buku</a>
    </nav>
    <div class="container">
        <div class="row py-5">
            <?php
                while($data = mysqli_fetch_array($result)){
                    echo "<div class='col-4 mb-4'>";
                    echo "<div class='card'>";
                    echo "<img src='images/$data[gambar]' height='250' style='object-fit: cover;width:100%;' class='card-img-top'>";
                    echo "<div class='card-body'>";
                    echo "<h2>".$data['judul_buku']."</h2>";
                    echo "<p>".$data['deskripsi']."</p>";
                    echo "<a href='detail.php?id=$data[id_buku]' class='btn btn-primary'>Tampilkan Lebih Lanjut</a>";
                    echo "</div>";
                    echo "</div>";
                    echo "</div>";
                }
                ?>
        </div>
    </div>
    <footer>
    <p>darryl_1202183344<br>
    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
</body>

</html>